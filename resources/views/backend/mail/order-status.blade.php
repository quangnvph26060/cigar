...
<body>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Đơn hàng của bạn đã {{ $order->order_status == 'confirmed' ? 'được' : 'bị' }}
                <strong>{{ $order->order_status == 'confirmed' ? 'xác nhận' : 'hủy' }}</strong>.
            </p>
            ...
            @if ($order->order_status == 'confirmed')
                ...
                <p><strong>Họ tên:</strong> {{ $order->username }}<br>
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach ($order->orderDetails as $product)
                                <tr>
                                    <td>{{ $product->p_name }}
                                        <small>x {{ $product->p_qty }}</small>
                                    </td>
                                    <td>{{ getFormattedSubTotal($product->p_price) }} $ / {{ $product->p_unit }}</td>
                                    <td>{{ getFormattedSubTotal($product->p_price * $product->p_qty) }} $
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <p><strong>Tổng tiền đơn hàng:</strong> {{ getFormattedSubTotal($order->total_amount) }} $</p>

                <span class="badge primary">Đơn hàng đã được xác nhận</span>
